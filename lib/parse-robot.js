...
*      ],
*      libraries: [
*          // 'path' is available only if this is a reference to a physicsl
*          // library (ie. Library  ../mylib.py). In this case 'name' will
*          // be 'mylib'.
*          {
*            physical: true/false,
*            path: 'libPath',
...
var parseLibraries = function(settingsSection){
  ...
  while(match = libraryRegexp.exec(settingsSection)){
    const library = match[1]?match[1].trim():undefined
    let path, name, physical;
    if(library.toLowerCase().endsWith('.py')){
      physical = true
      path = library
      const ext = pathUtils.extname(path)
      name = pathUtils.basename(path, ext)
    } else{
      physical = false
      name = library
    }
