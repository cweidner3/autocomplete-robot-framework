    ...
    if(!resourcePath){
      ...
      if(!currentResource){
        ...
        for(const libraryName of libraryNames){
          ...
          const managedLibrary = libManager.getLibrariesByKey().get(libraryName)
          if(managedLibrary){
...
function buildImportedLibrariesOutput(libraries, tabs){
  ...
  if(libraries.length==0){
    output.push(tab('No library imported', tabs+1))
  }
...
function buildImportedResourcesOutput(resources, tabs){
  ...
  if(resources.length==0){
    output.push(tab('No resource imported', tabs+1))
  }
