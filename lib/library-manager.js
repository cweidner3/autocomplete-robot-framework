...
function getLibraryKeys(){
  const libraryKeys = new Set(['BuiltIn'])
  for(resourceKey in keywordsRepo.resourcesMap){
    ...
    for(const library of resource.imports.libraries){
      if(library.physical && library.absolutePath){
        libraryKeys.add(library.absolutePath)
      } else if(!library.physical){
        libraryKeys.add(library.name)
      }
    }
  }

  return Array.from(libraryKeys)
}
...
/**
 * Parses for keywords libraries stored in library-repo.
 */
function parseManagedLibraries(){
  const libraries = libRepo.getLibrariesByKey().values()
  for(const library of libraries){
    if(library.status==='success'){
      ...
      if(isLibdocXmlFile(fileContent, libdocPath)){
        ...
        keywordsRepo.addLibrary(parsedRobotInfo, libdocPath, library.name,
          common.getResourceKey(library.libraryKey), library.sourcePath)
      }
      else{
        libRepo.updateLibrary({name: library.name, libraryKey: library.libraryKey, status: 'error', message: 'Not a valid Libdoc xml file'})
      }
    }
  }
}
...
**/
function loadFailedLibraries(libraryKeys){
  const libraries = []
  for(const libraryKey of libraryKeys){
    const fallbackLibrary = fallbackLibraries.get(libraryKey)
    if(fallbackLibrary){
      ...
      libraries.push({
        ...
        fallback: true,
        name: common.getLibraryName(libraryKey),
...
module.exports = {
  reset(){
    ...
    for(const libraryKey of libraryKeys){
      keywordsRepo.reset(libraryKey)
    }
    libRepo.reset()
    fallbackLibraries.clear()
  },
  // Imports all distinct libraries defined in any robot file.
  // Importer proceeds by transforming python libraries into libdoc equivalents
  // stored into libdoc cache dir.
  // libdocCacheDir: directory where importer will store libdoc files.
  // pythonExecutable: python command name or path
  importLibraries(libdocCacheDir, pythonExecutable){
    ...
    const libraryKeys = getLibraryKeys()
    try{
      libRepo.reset(libraryKeys)
      keywordsRepo.reset(libdocCacheDir)

      // Load and parse fallback libraries. We will override them later.
      loadFailedLibraries(libraryKeys)
      parseManagedLibraries()

      const moduleSearchPath = [] // TODO remove: getRobotDistinctPaths()
      return libRepo.importLibraries(libraryKeys, libdocCacheDir, moduleSearchPath, pythonExecutable)
        .then (()=>{
          parseManagedLibraries()
          if(importStatus==='busy'){
            ...
            importStatus = 'idle'
            return this.importLibraries(libdocCacheDir, pythonExecutable)
          }
        })
    } catch(error){
      const libraries = libRepo.getLibrariesByKey()
      const missingLibraries = []
      for(libraryKey of libraryKeys){
        if(!libraries.get(libraryKey)){
  ...
  getLibrariesByKey(libraryKey){
    return libRepo.getLibrariesByKey()
  },
