    ...
    if(library.status==='success'){
      ...
      if(isLibdocXmlFile(fileContent, libdocPath)){
        ...
        keywordsRepo.addLibrary(parsedRobotInfo, libdocPath, library.name,
          common.getResourceKey(library.resourceKey), library.sourcePath)
      }
      else{
...
module.exports = {
  ...
  importLibraries(libdocCacheDir, pythonExecutable){
    ...
    try{
      ...
      return libRepo.importLibraries(libraryNames, libdocCacheDir, moduleSearchPath, pythonExecutable)
      .then (()=>{
        parseManagedLibraries()
