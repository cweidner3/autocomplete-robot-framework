 ...
 *         imports: {
 *           libraries: [{
 *             // 'path' is defined only for physical libraries. It will not be
 *             // available if library could not be identified on disk.
 *             // If defined it holds the absolute normalized path to library.
 *             physical: true/false,
 *             path: '/a/b/libraryName',
 *             name: 'libraryName',
 *             alias: 'withNameAlias'}, ...],
 *           resources: [{
...
var addKeywords = function(parsedRobotInfo, resourceKey, path, name, isLibrary = false, libraryPath = undefined) {
  ...
  const importedLibraries = parsedRobotInfo.libraries.map(lib => {
    ...
    return {physical: lib.physical, path: libPath, name: lib.name, alias: lib.alias}
  })
